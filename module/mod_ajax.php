...
class mod_ajax extends ModuleHelper{
	...
	function dumpPost($no, $html=false)
	{
		global $PIO;
		$post = $PIO->fetchPosts($no)[0];
		$tree = array('post' => $no);
		$isThreadPost = $PIO->isThread($no);
		
		$re = $this->filterPost($post);
		if($html) 
			$re['html'] = $this->_dumpHtml($post, $tree, $isThreadPost);
	...
	function ModulePage(){
		...
		else if($action=='post')
		{
			...
			{
				$this->dumpPost(intval($_GET['no']), $html);
			}
			else
			{
				$this->output_error('invalid post number');
			}
		}
